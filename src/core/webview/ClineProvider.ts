...
export class ClineProvider implements vscode.WebviewViewProvider {
	public static readonly sideBarId = "cline-apipie.SidebarProvider"
	...
	*/
		...
		webview.onDidReceiveMessage(
			async (message: WebviewMessage) => {
				switch (message.type) {
					...
					case "clearMemory": {
						try {
							const sessionId = message.text || "cline-1"
							const { apiConfiguration } = await this.getState()
							
							if (apiConfiguration.apiProvider === "apipie" && apiConfiguration.apipieApiKey) {
								// Create a temporary APIpie handler to call clearMemory
								const apipieHandler = buildApiHandler({
									...apiConfiguration,
									apiProvider: "apipie"
								}) as any
								
								// Log the request
								console.log(`[APIpie] Clearing memory for session: ${sessionId}`)
								
								// Call the clearMemory method
								await apipieHandler.clearMemory(sessionId)
								
								vscode.window.showInformationMessage(`Memory cleared for session ${sessionId}`)
							} else {
								vscode.window.showErrorMessage("Cannot clear memory: APIpie provider not configured")
							}
						} catch (error) {
							console.error(`Failed to clear memory: ${error}`)
							vscode.window.showErrorMessage(`Failed to clear memory: ${error instanceof Error ? error.message : String(error)}`)
						}
						break
					}
	...
	async updateApiConfiguration(apiConfiguration: ApiConfiguration) {
		...
		await this.updateGlobalState("apipieMemory", apiConfiguration.apipieMemory)
		await this.updateGlobalState("apipieMemorySession", apiConfiguration.apipieMemorySession)
		await this.updateGlobalState("apipieMemoryExpire", apiConfiguration.apipieMemoryExpire)
		await this.updateGlobalState("apipieMemoryMsgs", apiConfiguration.apipieMemoryMsgs)
		await this.updateGlobalState("apipieIntegrity", apiConfiguration.apipieIntegrity)
